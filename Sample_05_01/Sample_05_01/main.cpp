...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	light.ptPostion = { 0.0f, 50.0f, 50.0f };
	...
	while (DispatchWindowMessage())
	{
		...
		light.ptPostion.x += g_pad[0]->GetLStickXF();

		if (g_pad[0]->IsPress(enButtonB))
		{
			light.ptPostion.y += g_pad[0]->GetLStickYF();
		}
		else
		{
			light.ptPostion.z += g_pad[0]->GetLStickYF();
		}

		//電球モデルのワールド行列を更新する
		lightModel.UpdateWorldMatrix(light.ptPostion, g_quatIdentity, g_vec3One);
